...
"VO vs(uint id:SV_VertexID){VO o;float2 q[3]={float2(-1,-1),float2(3,-1),float2(-1,3)};o.p=float4(q[id],0,1);return o;}\n"
...
int main(){
  ...
  HRESULT h=D3DCompile(SH,strlen(SH),0,0,0,"ps","ps_4_0",0,0,&pb,&e);if(e)printf("%s\n",(char*)e->GetBufferPointer());printf("ps compile hr=0x%08lx\n",h);
  ...
  D3D11_VIEWPORT vp={0,0,W,H,0,1};ctx->RSSetViewports(1,&vp);ctx->OMSetRenderTargets(2,rtv,0);
  ...
  int ok=1;unsigned char cc[2][4];
  for(int i=0;i<2;i++){D3D11_TEXTURE2D_DESC sd=rd;sd.BindFlags=0;sd.Usage=D3D11_USAGE_STAGING;sd.CPUAccessFlags=D3D11_CPU_ACCESS_READ;
    ID3D11Texture2D*stg=0;dev->CreateTexture2D(&sd,0,&stg);ctx->CopyResource(stg,rt[i]);
    D3D11_MAPPED_SUBRESOURCE m;if(FAILED(ctx->Map(stg,0,D3D11_MAP_READ,0,&m))){printf("map%d FAIL\n",i);return 2;}
    unsigned char*c=(unsigned char*)m.pData+32*m.RowPitch+32*4;cc[i][0]=c[0];cc[i][1]=c[1];cc[i][2]=c[2];ctx->Unmap(stg,0);}
  printf("RT0 RGB=%d,%d,%d (want green) RT1 RGB=%d,%d,%d (want magenta)\n",cc[0][0],cc[0][1],cc[0][2],cc[1][0],cc[1][1],cc[1][2]);
  ok=(cc[0][1]>200&&cc[0][0]<50)&&(cc[1][0]>200&&cc[1][2]>200&&cc[1][1]<50);
  printf(ok?"MRT_OK two render targets written\n":"MRT_FAIL\n");return ok?0:3;
}
