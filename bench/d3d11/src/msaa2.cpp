...
int main(){ID3D11Device*dev=0;ID3D11DeviceContext*ctx=0;D3D_FEATURE_LEVEL fl;
 ...
 float clr[4]={0,0,0.3f,1};ctx->ClearRenderTargetView(rtv,clr);D3D11_VIEWPORT vp={0,0,W,H,0,1};ctx->RSSetViewports(1,&vp);
   D3D11_RASTERIZER_DESC rsd={};rsd.FillMode=D3D11_FILL_SOLID;rsd.CullMode=D3D11_CULL_NONE;rsd.MultisampleEnable=TRUE;
  ID3D11RasterizerState*rs=0;dev->CreateRasterizerState(&rsd,&rs);ctx->RSSetState(rs);
 ...
 printf(ok?"MSAA_OK 4x render+resolve works (w/ raster state)\n":"MSAA_FAIL\n");return ok?0:4;}
